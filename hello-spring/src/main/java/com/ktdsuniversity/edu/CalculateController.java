...
public class CalculateController {
	...
	@GetMapping("/calc")
	public String viewCalcJspPage(Model model) {
	 ...
	 *        예> /calc2?firstNum=3&secondNum=10
	 ...
	 *        		</select>
	 *        		<textarea name="key2">Value</textarea>
	 *           </form>
	 *     3. Request Body - Http Request Body 영역에 파라미터를 보내는 방법
	 *        예> 파일 업로드, AJAX요청
	 *     4. Alternative - Complex. 여러가지를 섞어서 보내기도 함.
	 *        => Query String Parameter + Form Parameter
	 *        => Query String Parameter + Request Body
	 *     * Spring 전용 Parameter
	 *       => Path(URL Variable - Query String Parameter 외의 파라미터를 URL로 보내는 방법.
	 *  
	 *  *  Spring Endpoint에서 파라미터를 받아오는 4가지 방법
	 *      1. HttpServletRequest 객체를 이용하는 방법 (Spring에서는 잘 사용되지 않는다. IP, referrer 가져올 때 주로 사용)
	 ...
	 *      2. @RequestParam 을 이용하는 방법(종종 사용 됨 - 파라미터의 개수가 적을 때. 2개 이하)
	 *        => Query String Parameter, Form Parameter
	 *      3. @ModelAttribute를 이용하는 방법(가장 많이 사용됨 - 파라미터의 개수가 많을 때)
	 *        => Command Object라고도 부름.@ModelAttribute Annotation은 생략 가능 
	 *        => Query String Parameter, Form Parameter
