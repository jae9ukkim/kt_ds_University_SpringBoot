 ...
 * End point 를 생성하는 역할.
 */
@Controller // 해당 클래스가 엔드포인트를 만들 수 있도록 지원.
public class TestEndPointController {
	...
	// @Controller가 적용된 모든 클래스를 찾아 해당 클래스들은 인스턴스로 생성
	...
	public String viewHelloJspPage(Model model) {
		// Model model parameter ==> Template Engine(JSP 등)에게 데이터를 전송시키는 객체
		logger.debug("{}", model);
		// myname이라는 key로 "김재국"할당해서 템플릿에게 전달
		model.addAttribute("myname", "김재국");
		...
		// Controller에서 String type을 반환시켰을 경우
	 ...
	 */
	@GetMapping("/root")
	public ResponseEntity<String> firstWebPage() {
