...
public class SearchListVO {
	...
	// 페이지 그룹의 개수(페이지 개수 / pageCountInGroup)
	...
	/**
	 * 조회된 게시글의 개수와 listSize를 이요해 총 몇 개의 페이지가 필요한 지 계산
	 * @param articleCount 게시글의 매수
	 */
	public void computePagination(int articleCount) {
		...
		this.pageGroupCount = (int)Math.ceil((double)this.pageCount / this.pageCountInGroup);
		// 현재 페이지 그룹 번호 계산
