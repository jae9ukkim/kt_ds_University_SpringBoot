...
public class FilesController {
	...
	public ResponseEntity<Resource> doDownloadAction(@PathVariable String fileGroupId, @PathVariable int fileNum) {
		...
		// HttpHeaders 에 파일을 첨부해서 보내겠다(attachment)
		...
		headers.set(HttpHeaders.CONTENT_LENGTH, downloadVO.getFileLength() + ""); // set은 String type만 받는다
		// Content-Type : 다운로드할 파일의 마임타입(Mime-Type) 작성
		headers.set(HttpHeaders.CONTENT_TYPE, 
				this.mimeTypeMap.getOrDefault(downloadVO.getExtendName().toLowerCase(), "application/octet-stream"));
