 ...
 *  9. 회원 탈퇴 처리 URL
 *  10. 회원 로그아웃 URL
 *  위 URL 10개에 세션없이 접근할 경우(Controller가 실행되기 전) 로그인 페이지로 이동시키기 
 ...
 *  7. 게시글 내용 조회 페이지
 *  8. 이메일 중복 검사
 *  ==> 위 URL 1 ~ 4 까지는 세션이 있는 상태에서 접근을 못해야 한다.
 */
public class SessionInterceptor implements HandlerInterceptor{ // interceptor의 기능을 정의해둔 인터페이스

	/**
	 * 컨트롤러가 실행되기 이전에 수행할 공통코드를 작성하는 영역
	 * @param request 브라우저(클라이언트)가 요청한 내용 - header, parameter, file, session, ...
	 * @param response 컨트롤러가 실행되기 이전에 브라우저(클라이언트)로 응답을 보내는 역할
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		if(session.getAttribute("__LOGIN_DATA__") == null) {
			...
			if(pathname.startsWith("/api/")) {
				...
				// 클라이언트에게 데이터(HTML, CSS, JSON, Javascript, Image 등 전송하는 객체)
			...
			//	  대신, 사용자에게 로그인 페이지를 보여주도록 한다. URL은 변동되지 않아야 한다.
			//	  예> URL => 게시글 수정 처리, 보여지는 페이지는 로그인 페이지
	 ...
	 * 
	 * @param request 브라우저(클라이언트)가 요청한 내용 - header, parameter, file, session, ...
	 * @param response response 컨트롤러가 실행된 후에 브라우저(클라이언트)로 응답을 보내는 역할(필요시 사용)
