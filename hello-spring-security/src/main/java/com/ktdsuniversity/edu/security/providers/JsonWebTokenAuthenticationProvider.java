 ...
 * Spring Security AuthenticationProvicer와는 무관
...
public class JsonWebTokenAuthenticationProvider {
	...
	/**
	 * 사용자가 요청할 때 마다 Request Header[Authorization]에 전달한
	...
	public String decryptJsonWebToken(String jsonWebToken) {
		...
		Claims claims = Jwts.parser() // JsonWebToken을 분석하기 위한 선언
							...
							.requireIssuer(this.issuer) // 사용자가 전달한 JsonWebToken이 hello-spring-security 시스템에서 만든 것인지 확인
	...
	public String makeJsonWebToken(String email, Duration expiredAt) {
		...
		String jsonWebToken = Jwts.builder()
								...
								// JsonWebToken에 포함되어야 할 회원의 정보들(개인정보 들어가면 위험. 최소한으로)
								.claim("identify", email)
								// JsonWebToken을 발행한 날짜와 시간
								.issuedAt(issueDate)
								// JsonWebToken이 만료되는 날짜아 시간
								.expiration(expirationDate)
								// 평문으로 구성된 JsonWebToken을 암호화 또는 복호화 시킬 때 사용할 키
								.signWith(signKey)
	...
	public static void main(String[] args) {
		JsonWebTokenAuthenticationProvider jwtProvider = new JsonWebTokenAuthenticationProvider("dfgekrjthioq23r90thrfg90h94gmdl3-)_)$sffmo1","hello-spring-security");
