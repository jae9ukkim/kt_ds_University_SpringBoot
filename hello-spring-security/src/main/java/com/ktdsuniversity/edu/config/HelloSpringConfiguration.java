...
// @Component 의 자식 Annotation - bean container에 적재될 수 있다
@Configuration
...
// Spring Security의 필터목록을 확인하기 위해서 작성한다(debug = true)
@EnableWebSecurity(debug = false)
// Controller 혹은 Service 코드에서 권한 검사를 수행하기 위한 annotation 추가(반드시 필요)
...
public class HelloSpringConfiguration implements 
	...
	// @Value가 동작하는 조건 : @Component가 적용된 클래스에서만 가능
	...
	PasswordEncoder createPasswordEncoder() {
		return new SecurityPasswordEncoder(); // 생성자. 객체반환.
	}
	
	// SecurityUserDetailsService의  Bean을 생성한다.
	@Bean // @Bean으로 생성하는 객체(bean)들은 필요한 의존 객체를 생성자/setter로 직접 주입해 주어야 한다.
	...
	@Bean
	AuthenticationProvider createUserAuthenticationProvider() {
	 ...
	 * --------------------------------------------------------------------
	 * Controller에서 해당 페이지를 노출하려 할 때 "/WEB-INF/vies/.../*.jsp"경로 사용 시
	...
	SecurityFilterChain configureFilterChain(HttpSecurity httpSecurity) {
		...
		httpSecurity.cors(corsConfigurer -> {
			...
			CorsConfigurationSource source = (httpServletRequest) -> {
				...
				config.addAllowedOrigin("http://192.168.211.26:3000");
